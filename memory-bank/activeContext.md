...
- **PARALLEL SESSION (Jun 27) — climb name/parent resolver, see `note-climb-resolution-producer-jun27.md`:** a worksheet session minted ~992 `canonical_persons` (`created_by='climb_name_resolver'`) OUTSIDE the identity layer, then RECONCILED them with this session's PersonService (`scripts/reconcile-climb-minted.js`): blocking keys backfilled 970/992 (silo closed), Biscoe dedup found 0 real duplicates (28 ambiguous → `worksheets/dedup-candidates.json`, not merged). STILL OPEN: (a) they're minted canonical not gated leads (but M102 gate defaults them FALSE = hidden/non-assertable, so practically safe); (b) parent edges live in `inferred_parent_links`, NOT `canonical_family_edges` (M103) → #3 can't traverse them yet; (c) route `resolve-climb-ancestors.js` + `scrape-parents.js` through PersonService going forward. Those producer scripts are untracked (uncommitted) — coordinate before committing/running.
